...
public interface PurchaseRepository extends JpaRepository<Purchase, Integer> {
	...
	@Query("SELECT p FROM Purchase p WHERE p.warehouse.id = :wId AND p.product.id = :pId AND p.remainingQuantity > 0 ORDER BY p.purchasedAt ASC")
	...
	@Query("SELECT p FROM Purchase p WHERE p.warehouse.id = :wId AND p.product.id = :pId ORDER BY p.purchasedAt ASC")
