...
const ProductStats = ({ warehouses }) => {
    ...
    } = useProductStats(selectedWarehouseId);
                    ...
                    <>
                        <Paper elevation={1} sx={{ p: 2, borderRadius: 2 }}>
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                                <Box sx={{ width: '450px' }}>
                                    ...
                                    {!selectedProduct ? (
                                        <Box sx={{ height: '40px' }}><ProductSearch warehouseId={selectedWarehouseId} onProductSelect={setSelectedProduct} /></Box>
                                    ) : (
                                        <TextField fullWidth size="small" value={selectedProduct.name}
                                            sx={{ '& .MuiInputBase-root': { height: '40px', bgcolor: '#f5f5f5', fontWeight: 'bold' } }}
                        ...
                        <Box sx={{ flex: 1, overflowY: 'auto' }}>
                            ...
                            {selectedProduct && !loading && productTimeline?.groups?.map((group, index) => {
                                const nextGroup = productTimeline.groups[index + 1];
